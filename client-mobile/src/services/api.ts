...
export const bannerApi = {
  getBanners: async (): Promise<Banner[]> => {
    const response =
      await apiClient.get<ApiResponse<{ banners: Banner[] }>>(
        "/mobile/banners",
      );
...
export const hotelApi = {
  searchHotels: async (
    ...
    const response = await apiClient.get<ApiResponse<HotelListResponse>>(
      "/mobile/hotels/search",
      {
        params,
      },
    );
  ...
  getHotelDetail: async (id: number): Promise<HotelDetail> => {
    const response = await apiClient.get<ApiResponse<HotelDetail>>(
      `/mobile/hotels/${id}`,
    );
...
export const filterApi = {
  ...
  getQuickTags: async (): Promise<QuickTagsResponse> => {
    const response = await apiClient.get<ApiResponse<QuickTagsResponse>>(
      "/mobile/filters/tags",
    );
    return response.data.data;
  },
...
export const cityApi = {
  getCities: async (): Promise<string[]> => {
    const response =
      await apiClient.get<ApiResponse<CitiesResponse>>("/mobile/cities");
