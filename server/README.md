# 易宿酒店预订平台 - 后端服务

基于 Node.js + Express + Prisma 的酒店管理平台后端服务，提供商户端酒店管理和管理员端审核发布功能。

## 功能特性

- 🔐 用户认证与授权（JWT）
- 🏨 酒店信息管理（商户端）
- ✅ 酒店审核发布（管理员端）
- 📷 图片上传功能
- 📝 完整的数据校验

## 快速开始

### 环境要求

- Node.js >= 16.x
- MySQL >= 5.7

### 安装依赖

```bash
npm install
```

### 配置环境变量

创建 `.env` 文件：

```env
DATABASE_URL="mysql://root@localhost:3306/yisu-hotel-db"
JWT_SECRET="your-secret-key-here"
PORT=3000
```

### 初始化数据库

```bash
npm run prisma:generate
npm run prisma:migrate
```

### 启动服务

```bash
npm start
```

服务将在 `http://localhost:3000` 启动。

## API 基础信息

- **Base URL**: `http://localhost:3000/api/v1`
- **认证方式**: Bearer Token (JWT)
- **数据格式**: JSON

## 接口列表

### 认证模块

| 接口     | 方法 | 路径           | 说明     | 权限 |
| -------- | ---- | -------------- | -------- | ---- |
| 用户注册 | POST | /auth/register | 用户注册 | 公开 |
| 用户登录 | POST | /auth/login    | 用户登录 | 公开 |

### 酒店管理模块（商户端）

| 接口            | 方法 | 路径              | 说明               | 权限     |
| --------------- | ---- | ----------------- | ------------------ | -------- |
| 获取/初始化酒店 | GET  | /hotels/my        | 获取商户的酒店信息 | merchant |
| 更新酒店信息    | PUT  | /hotels/my        | 更新酒店信息       | merchant |
| 提交审核        | PUT  | /hotels/my/submit | 提交酒店审核       | merchant |
| 取消提交        | PUT  | /hotels/my/cancel | 取消提交审核       | merchant |

### 酒店审核模块（管理员端）

| 接口         | 方法 | 路径                      | 说明             | 权限  |
| ------------ | ---- | ------------------------- | ---------------- | ----- |
| 获取酒店列表 | GET  | /admin/hotels             | 获取所有酒店列表 | admin |
| 获取酒店详情 | GET  | /admin/hotels/:id         | 获取酒店详细信息 | admin |
| 审核通过     | PUT  | /admin/hotels/:id/approve | 审核通过         | admin |
| 审核不通过   | PUT  | /admin/hotels/:id/reject  | 审核不通过       | admin |
| 发布酒店     | PUT  | /admin/hotels/:id/publish | 发布酒店上线     | admin |
| 下线酒店     | PUT  | /admin/hotels/:id/offline | 下线酒店         | admin |
| 恢复上线     | PUT  | /admin/hotels/:id/restore | 恢复已下线酒店   | admin |

### 文件上传模块

| 接口     | 方法 | 路径           | 说明         | 权限           |
| -------- | ---- | -------------- | ------------ | -------------- |
| 单图上传 | POST | /upload/image  | 上传单张图片 | merchant/admin |
| 多图上传 | POST | /upload/images | 上传多张图片 | merchant/admin |

## 酒店状态流转

```
draft ──提交审核──> pending ──审核通过──> approved ──发布──> published
  ↑                    │                                 │
  │                    └──审核不通过──> rejected          │
  │                                         │            │
  └────────────编辑──────────────────────────┘            │
                                                          │
                        offline <──────下线───────────────┘
                          │
                          └────────恢复──> published
```

## 统一响应格式

所有接口返回统一的信封格式：

```json
{
  "code": 200,
  "msg": "操作成功",
  "data": {}
}
```

## 开发命令

```bash
# 启动服务器
npm start

# 生成 Prisma Client
npm run prisma:generate

# 运行数据库迁移
npm run prisma:migrate

# 打开 Prisma Studio(在浏览器中查看数据库)
npm run prisma:studio
```

## 技术栈

- **Node.js** - 运行时环境
- **Express** - Web 框架
- **Prisma** - ORM 框架
- **MySQL** - 数据库
- **JWT** - 身份认证
- **bcryptjs** - 密码加密
- **multer** - 文件上传

## 项目结构

```
server/
├── app.js                 # 应用入口
├── package.json           # 项目配置
├── types.ts              # TypeScript 类型定义
├── .env                  # 环境变量
├── uploads/              # 上传文件目录
├── prisma/               # Prisma 配置
│   └── schema.prisma     # 数据模型定义
├── controllers/          # 控制器层
│   ├── userController.js
│   ├── hotelController.js
│   ├── adminController.js
│   └── uploadController.js
├── middleware/           # 中间件
│   └── auth.js
├── routes/               # 路由层
│   ├── auth.js
│   ├── hotel.js
│   ├── admin.js
│   └── upload.js
├── utils/                # 工具函数
│   ├── jwt.js
│   ├── response.js
│   └── upload.js
├── constants/            # 常量定义
│   └── response.js
└── docs/                 # 文档
    ├── API_DESIGN.md
    └── DATABASE_DESIGN.md
```

## 文档

- [API 设计文档](./docs/API_DESIGN.md) - 完整的接口设计文档
- [数据库设计文档](./docs/DATABASE_DESIGN.md) - 数据库表结构设计

## 安全特性

- 密码使用 bcrypt 加密存储
- JWT 令牌认证，有效期 24 小时
- 统一的数据校验和错误处理
- SQL 注入防护（Prisma ORM）
- 文件上传类型和大小限制

## 许可证

MIT
